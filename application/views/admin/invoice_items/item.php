            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div id="custom_fields_items_primary">
                            <?php
                            // CUSTOM ORDER FOR ITEM FIELDS - DO NOT OVERWRITE
                            echo render_custom_fields('items', false, [], [
                                'include_field_names' => ['Part Name', 'Part No', 'Alt No'],
                            ]);
                            ?>
                        </div>
...
?>
                        ...
                        <div id="custom_fields_items_secondary">
                            <?php
                            echo render_custom_fields('items', false, [], [
                                'include_field_names' => ['Condition', 'Lead time'],
                            ]);
                            ?>
                        </div>
                        ...
                        <div id="custom_fields_items">
                            <?= render_custom_fields('items', false, [], [
                                'exclude_field_names' => $item_custom_field_order,
                            ]); ?>
                        </div>
...
<script>
    ...
    function init_item_js() {
        function setCustomFieldValue($itemModal, field) {
            var baseName = 'custom_fields[items][' + field.id + ']';
            var value = field.value === null ? '' : field.value;

            if (field.type === 'checkbox') {
                var values = value === '' ? [] : value.split(',').map(function(option) {
                    return option.trim();
                });

                $itemModal.find('input[name="' + baseName + '[]"]').each(function() {
            ...
            if (field.type === 'multiselect') {
                $itemModal.find('select[name="' + baseName + '[]"]').selectpicker('val', value === '' ? [] : value.split(',').map(function(option) {
                    return option.trim();
                }));
                return;
            }
            ...
            if (field.type === 'select') {
                $itemModal.find('select[name="' + baseName + '"]').selectpicker('val', value);
                return;
            }

            if (field.type === 'textarea') {
                $itemModal.find('textarea[name="' + baseName + '"]').val(value);
                return;
            }

            $itemModal.find('[name="' + baseName + '"]').val(value);
        }

        function resetItemCustomFields($itemModal) {
            $itemModal.find('[name^="custom_fields[items]"]').each(function() {
                var $field = $(this);
        ...
        function populateItemCustomFields($itemModal, customFields) {
            $.each(customFields, function(_, field) {
                setCustomFieldValue($itemModal, field);
            });
        ...
        $("body").on('show.bs.modal', '#sales_item_modal', function(event) {
            ...
            resetItemCustomFields($itemModal);
            ...
            if (typeof(id) !== 'undefined') {
                ...
                requestGetJSON('invoice_items/get_item_by_id/' + id).done(function(response) {
                    ...
                    populateItemCustomFields($itemModal, response.custom_fields);
