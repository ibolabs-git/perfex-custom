...
function render_custom_fields($belongs_to, $rel_id = false, $where = [], $items_cf_params = [])
{
    ...
    $include_field_names = isset($items_cf_params['include_field_names']) && is_array($items_cf_params['include_field_names'])
        ? $items_cf_params['include_field_names']
        : [];
    $exclude_field_names = isset($items_cf_params['exclude_field_names']) && is_array($items_cf_params['exclude_field_names'])
        ? $items_cf_params['exclude_field_names']
        : [];
    ...
    if (count($include_field_names) > 0 || count($exclude_field_names) > 0) {
        $normalize_field_name = function ($name) {
            return strtolower(trim(strip_tags((string) $name)));
        };

        $include_field_names = array_map($normalize_field_name, $include_field_names);
        $exclude_field_names = array_map($normalize_field_name, $exclude_field_names);

        $fields = array_values(array_filter($fields, function ($field) use ($include_field_names, $exclude_field_names, $normalize_field_name) {
            $normalized_name = $normalize_field_name($field['name']);

            if (count($include_field_names) > 0 && !in_array($normalized_name, $include_field_names, true)) {
                return false;
            }

            if (count($exclude_field_names) > 0 && in_array($normalized_name, $exclude_field_names, true)) {
                return false;
            }
